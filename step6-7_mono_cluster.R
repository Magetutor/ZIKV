...
# 单核细胞亚分群
# 挑选T细胞进⾏进⼀步分群
target.cell1 <- row.names(zika_all_mthb_filter_SCT@meta.data)[which(zika_all_mthb_filter_SCT@meta.data$cell_type == "Monocyte_Lyz2")]
target.cell2 <- row.names(zika_all_mthb_filter_SCT@meta.data)[which(zika_all_mthb_filter_SCT@meta.data$cell_type == "Monocytes & Neutrophils")]
target.cell <- c(target.cell1,target.cell2)
length(target.cell)
Mono.dat <- subset(zika_all_mthb_filter_SCT, cells = target.cell)
Mono.dat
DefaultAssay(Mono.data) <- "RNA"
Mono.data <- SplitObject(Mono.data,
                      split.by = "Group")
Mono.data

# 还是先看一下线粒体这些,整合之后才可以看
# VlnPlot(object = Mono.data,
...
# 速度较慢，样本越多越慢
# 循环运⾏
for(i in 1:length(Mono.data)){
  Mono.data[[i]] <- SCTransform(
    Mono.data[[i]],
...
# 数据整合
# 选择⽤于整合的基因
# load("Mono.data.SCT.Rda")
features <- SelectIntegrationFeatures(object.list = Mono.data,
                                      nfeatures = 3000)
# 准备整合
Mono.data <- PrepSCTIntegration(object.list = Mono.data,
                             anchor.features = features)
names(Mono.data)
# 寻找锚定基因(速度很慢)
AnchorSet <- FindIntegrationAnchors(object.list = Mono.data,
                                    reference = 3,
...
# 整合数据
# load("AnchorSet.Rda")
Mono.data <- IntegrateData(anchorset = AnchorSet,
                        normalization.method = "SCT")
DefaultAssay(Mono.data) <- "integrated"
save(Mono.data, file = "Mono.data.final.Rda")
############降维分析##########
rm(list = ls())
library(Seurat)
library(tidyverse)
library(stringr)
library(patchwork)
load("Mono.data.final.Rda")
dim(Mono.data)
# PCA分析
Mono.data <- RunPCA(object = Mono.data,
                 npcs = 50,
                 rev.pca = FALSE,
                 weight.by.var = TRUE,
...
Mono.data <- RunTSNE(Mono.data,
                  dims = 1:30)
# UMAP分析
Mono.data <- RunUMAP(Mono.data,
                  dims = 1:30)
# 展示PCA降维特征及其特征基因的热图
DimHeatmap(object = Mono.data,
           reduction = "pca",
           dims = c(1:5),
           nfeatures = 30,
           disp.min = -2.5,
...
##########聚类分析##########
# KNN聚类法
Mono.data <- FindNeighbors(Mono.data,
                        k.param = 20, # 不要⼩于5
                        dims = 1:30)
Mono.data <- FindClusters(Mono.data,
                       resolution = 0.1,
                       method = "igraph",
                       algorithm = 1,
                       random.seed = 2021)
table(Mono.data@meta.data$seurat_clusters)

PCAPlot(object = Mono.data,
        group.by = "seurat_clusters",
        pt.size = 0.5,
        label = TRUE)
TSNEPlot(object = Mono.data,
         group.by = "seurat_clusters",
         pt.size = 0.5,
         label = TRUE)
UMAPPlot(object = Mono.data,
         group.by = "seurat_clusters",
         pt.size = 0.5,
         label = TRUE)

# 保存一下分好群的数据
save(Mono.data, file = "Mono.data_SCT_cluster3.Rda")

# 计算每个cluster的差异基因
# 没有数据的话需要load('Mono.data_SCT_cluster5.Rda')

logFCfilter=0.5
adjPvalFilter=0.05
Mono.data.markers <- FindAllMarkers(object = Mono.data,
                                 only.pos = TRUE, #TRUE表示只求高表达的基因
                                 min.pct = 0.25, #表达的最小百分比
                                 logfc.threshold = logFCfilter) # 这一步花了1-2h
# 把marker文件保存下来
save(Mono.data.markers, file = "Mono.data.markers_cluster3.Rda")

sig.markers = Mono.data.markers[(abs(as.numeric(as.vector(Mono.data.markers$avg_log2FC)))>logFCfilter & as.numeric(as.vector(Mono.data.markers$p_val_adj))<adjPvalFilter),]
write.table (sig.markers,file="PC30_mono.markers.xls",sep="\t",row.names=F,quote=F)
top5 <- Mono.data.markers %>% group_by(cluster) %>% top_n(n = 5, wt = avg_log2FC)
top10 <- Mono.data.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
top20 <- Mono.data.markers %>% group_by(cluster) %>% top_n(n = 20, wt = avg_log2FC)
write.table (top20 ,file="PC30_top20_mono.markers.xls",sep="\t",row.names=F,quote=F)


#绘制marker在各个cluster的热图
pdf(file="09.tsneHeatmap.pdf",width=32,height=8)
DoHeatmap(object = Mono.data, features = top10$gene,size = 3,angle = 0) + NoLegend()
dev.off()  # 绘制速度较慢

FeaturePlot(object = Mono.data, features = c("Irak3", "Bcl2a1b"))
FeaturePlot(object = Mono.data, features = c( 'Ier3', 'Fos'),
            reduction = "umap")
VlnPlot(Mono.data,
...
# 先给每个群命名一下
####写入细胞类型
Cell_type1 <- c("0" = "unknow",
               "1" = "Flna_mono",
               "2" = "Tcf4_mono"
) 

#将定义各个亚型对应的细胞名称，然后写⼊到meta.data⽂件
Mono.data[['cell_type1']] <- unname(Cell_type1[Mono.data@meta.data$seurat_clusters])

#看一下结果
View(Mono.data@meta.data)
...
DimPlot(Mono.data,
  ...
  NoLegend()
#保存一下注释好的数据
save(Mono.data, file = "Mono.data_3cluster_ann.Rda")
#### 保存细胞亚型 ----
saveRDS(Mono.data, file = "mono.rds")
...
# 
# # 先给定细胞身份
# Idents(Mono.data) <- Mono.data@meta.data$cell_type
# levels(Mono.data)
# table(Mono.data@meta.data$cell_type)
# 
# View(Mono.data@meta.data)
# Macro_Apoe_high2 <- FindConservedMarkers(Mono.data,
...
load('/home/zhangc/analysis/zika/data/mono/Mono.data_3cluster_ann.Rda')
########## 单核细胞亚群的差异基因分析#############
# 批量找各个细胞在VFS中的差异基因
dput(unique(Mono.data@meta.data$cell_type1))
table(Mono.data@meta.data$cell_type1)
Cells <- c("unknow", "Flna_mono", "Tcf4_mono")
diff_VFS_mono <- list()
for (i in 1:length(Cells)) {
  cells <- subset(Mono.data,
                  cell_type1 == Cells[i])
  ...
  diff_VFS_mono [[i]] <- cells.diff
}
names(diff_VFS_mono ) <- Cells
save(diff_VFS_mono , file = "diff_VFS_mono_alldiff.Rda")
...
length(diff_VFS_mono)
rownames(diff_VFS_mono[[1]])
ENTREZID <- bitr(rownames(diff_VFS_mono[[1]][diff_VFS_mono[[1]]$p_val_adj < 0.05,]), 
                 fromType = "SYMBOL", 
                 toType = "ENTREZID", 
                 OrgDb = "org.Mm.eg.db")
...
# 批量转换
scRNA.diff_VFS_mono <- list()
for (i in 1:length(diff_VFS_mono)) {
  ENTREZID <- bitr(rownames(diff_VFS_mono[[i]][diff_VFS_mono[[i]]$p_val_adj < 0.05,]), 
                   fromType = "SYMBOL", 
                   toType = "ENTREZID", 
                   OrgDb = "org.Mm.eg.db")
  ENTREZID <- ENTREZID$ENTREZID
  scRNA.diff_VFS_mono[[i]] <- ENTREZID
}
names(scRNA.diff_VFS_mono) <- names(diff_VFS_mono)

############## 多个cluster比较 ###################
# KEGG分析
xx <- compareCluster(scRNA.diff_VFS_mono, 
                     fun="enrichKEGG",
                     organism="mmu",
                     pvalueCutoff=0.1,
                     qvalueCutoff=0.1)
xx <- pairwise_termsim(xx) # 获取相似性矩阵
dotplot(xx, showCategory = 5, color = 'pvalue') #,label_format = 60 可以修改通路一行还是两行
...
xx_BP <- compareCluster(scRNA.diff_VFS_mono, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "BP",
                        pvalueCutoff=0.001,
...
xx_MF <- compareCluster(scRNA.diff_VFS_mono, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "MF",
                        pvalueCutoff=0.01,
...
xx_CC <- compareCluster(scRNA.diff_VFS_mono, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "CC",
                        pvalueCutoff=0.01,
...
diffgene_flna <- scRNA.diff_VFS_mono[[2]]
...
diffgene_Tcf4 <- scRNA.diff_VFS_mono[[3]]
...
diffgene_unknow <- scRNA.diff_VFS_mono[[1]]
...
#获得基因列表
gene <- rownames(diff_VFS_mono[[5]])
## 转换
gene = bitr(gene, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Mm.eg.db")
## 去重
gene <- dplyr::distinct(gene,SYMBOL,.keep_all=TRUE)
gene_df <- data.frame(logFC=diff_VFS_mono[[5]]$avg_log2FC,
                     SYMBOL = rownames(diff_VFS_mono[[5]]))
...
for (i in 1:length(Cells)) {
  ...
  diff_V59_mono [[i]] <- cells.diff
}
names(diff_V59_mono ) <- Cells
save(diff_V59_mono , file = "diff_V59_mono_alldiff.Rda")
...
length(diff_V59_mono)
rownames(diff_V59_mono[[1]])
ENTREZID <- bitr(rownames(diff_V59_mono[[1]]), 
                 fromType = "SYMBOL", 
                 toType = "ENTREZID", 
                 OrgDb = "org.Mm.eg.db")
Irak3_NK.diff <- list(ENTREZID)
...
# 批量转换
scRNA.diff_V59_mono <- list()
for (i in 1:length(diff_V59_mono)) {
  ENTREZID <- bitr(rownames(diff_V59_mono[[i]]), 
                   fromType = "SYMBOL", 
                   toType = "ENTREZID", 
                   OrgDb = "org.Mm.eg.db")
  ENTREZID <- ENTREZID$ENTREZID
  scRNA.diff_V59_mono[[i]] <- ENTREZID
}
names(scRNA.diff_V59_mono) <- names(diff_V59_mono)

# 多个cluster比较
# KEGG分析
xx <- compareCluster(scRNA.diff_V59_mono, 
                     fun="enrichKEGG",
                     organism="mmu",
                     pvalueCutoff=0.05,
                     qvalueCutoff=0.05)
...
# GO分析
# all 整体分析会有NA出现，单独分析看看
xx_BP <- compareCluster(scRNA.diff_V59_mono, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "BP",
                        pvalueCutoff=0.000001,
                        qvalueCutoff=0.0000001)
#save(xx, file = "compareCluster.GO.Rda")
...
xx_MF <- compareCluster(scRNA.diff_V59_mono, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "MF",
                        pvalueCutoff=0.001,
...
xx_CC <- compareCluster(scRNA.diff_V59_mono, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "CC",
                        pvalueCutoff=0.000001,
...
dotplot(xx_CC, showCategory = 5) +
  ...
  theme(axis.text.x = element_text(size = 15,  vjust = 1, 
        ...
        axis.text.y = element_text(size = 12,  vjust = 1, 
                                   hjust = 1, angle = -8)) # color = "darkred",
# ggsave("compareCluster.GO.pdf")4

################细胞亚型数量及比例##############
plot.data <- Mono.data@meta.data[c("Group", "cell_type1")]
head(plot.data)
