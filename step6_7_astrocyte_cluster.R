...
# 单核细胞亚分群
# 挑选T细胞进⾏进⼀步分群
astro1 <- row.names(zika_all_mthb_filter_SCT@meta.data)[which(zika_all_mthb_filter_SCT@meta.data$cell_type == "Astrocyte_Fabp7")]
astro2 <- row.names(zika_all_mthb_filter_SCT@meta.data)[which(zika_all_mthb_filter_SCT@meta.data$cell_type == "Astrocyte_Slc1a3")]

target.cell <- c(astro1,astro2)
length(target.cell)
astro.data <- subset(zika_all_mthb_filter_SCT, cells = target.cell)
astro.data
DefaultAssay(astro.data) <- "RNA"
astro.data <- SplitObject(astro.data,
                          split.by = "Group")
astro.data
...
# 速度较慢，样本越多越慢
# 循环运⾏
for(i in 1:length(astro.data)){
  astro.data[[i]] <- SCTransform(
    astro.data[[i]],
...
# 数据整合
# 选择⽤于整合的基因
features <- SelectIntegrationFeatures(object.list = astro.data,
                                      nfeatures = 3000)
# 准备整合
astro.data <- PrepSCTIntegration(object.list = astro.data,
                                 anchor.features = features)
names(astro.data)
# 寻找锚定基因(速度很慢)
AnchorSet <- FindIntegrationAnchors(object.list = astro.data,
                                    reference = 3,
...
# 整合数据
# load("AnchorSet.Rda")
astro.data <- IntegrateData(anchorset = AnchorSet,
                            normalization.method = "SCT")
DefaultAssay(astro.data) <- "integrated"
save(astro.data, file = "astro.data.final.Rda")
############降维分析##########
rm(list = ls())
library(Seurat)
library(tidyverse)
library(stringr)
library(patchwork)
load("astro.data.final.Rda")
dim(astro.data)
# PCA分析
astro.data <- RunPCA(object = astro.data,
                     npcs = 50,
                     rev.pca = FALSE,
                     weight.by.var = TRUE,
...
ElbowPlot(astro.data,
          ndims = 50) # 寻找对细胞差异贡献度较⼤的主成分
# 选择30个主成分进⾏后续分析
# t-SNE分析，基于PCA分析结果进⾏⾮线性降维分析
astro.data <- RunTSNE(astro.data,
                      dims = 1:30)
# UMAP分析
astro.data <- RunUMAP(astro.data,
                      dims = 1:30)
# 展示PCA降维特征及其特征基因的热图
DimHeatmap(object = astro.data,
           reduction = "pca",
           dims = c(1:5),
           nfeatures = 30,
           disp.min = -2.5,
...
FeaturePlot(astro.data,
            reduction = "umap",
            features = "Cxcl2")
save(astro.data, file = "astro.data.before_cluster.Rda")
##########聚类分析##########
# KNN聚类法
astro.data <- FindNeighbors(astro.data,
                            k.param = 20, # 不要⼩于5
                            dims = 1:30)
astro.data <- FindClusters(astro.data,
                           resolution = 0.1,
                           method = "igraph",
                           algorithm = 1,
                           random.seed = 2021)
table(astro.data@meta.data$seurat_clusters)

PCAPlot(object = astro.data,
        group.by = "seurat_clusters",
        pt.size = 0.5,
        label = TRUE)
TSNEPlot(object = astro.data,
         group.by = "seurat_clusters",
         pt.size = 0.5,
         label = TRUE)
UMAPPlot(object = astro.data,
         group.by = "seurat_clusters",
         pt.size = 0.5,
         label = TRUE)
...
save(astro.data, file = "astro.data_SCT_cluster6.Rda")


# 没有数据的话需要load('astro.data_SCT_cluster8.Rda')
logFCfilter=0.5
adjPvalFilter=0.05
astro.data.markers <- FindAllMarkers(object = astro.data,
...
# 把marker文件
保存下来
save(astro.data.markers, file = "astro.data.markers_cluster6.Rda")

sig.markers = astro.data.markers[(abs(as.numeric(as.vector(astro.data.markers$avg_log2FC)))>logFCfilter & as.numeric(as.vector(astro.data.markers$p_val_adj))<adjPvalFilter),]
write.table (sig.markers,file="PC30_astro.markers_cluster6.xls",sep="\t",row.names=F,quote=F)
top5 <- astro.data.markers %>% group_by(cluster) %>% top_n(n = 5, wt = avg_log2FC)
top10 <- astro.data.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
top20 <- astro.data.markers %>% group_by(cluster) %>% top_n(n = 20, wt = avg_log2FC)
...
DoHeatmap(object = astro.data, features = top10$gene,size = 3,angle = 0) + NoLegend()
dev.off()  # 绘制速度较慢

FeaturePlot(object = astro.data, features = c("Irak3", "Bcl2a1b"))
FeaturePlot(object = astro.data, features = c( 'Ier3', 'Fos'),
            reduction = "umap")
VlnPlot(astro.data,
...
astro.data[['cell_type1']] <- unname(Cell_type1[astro.data@meta.data$seurat_clusters])

#看一下结果
View(astro.data@meta.data)
...
DimPlot(astro.data,
  ...
  NoLegend()
#保存一下注释好的数据
save(astro.data, file = "astro.data_6cluster_ann.Rda")
load('/home/zhangc/analysis/zika/data/step6_7_astro/astro.data_6cluster_ann.Rda')

#### 保存细胞亚型 ----
saveRDS(astro.data, file = "astro_6cluster_ann.rds")

########## 单核细胞亚群的差异基因分析#############
# 批量找各个细胞在VFS中的差异基因
dput(unique(astro.data@meta.data$cell_type1))
table(astro.data@meta.data$cell_type1)
Cells <- c("unkown", "Dynlrb2", "Lgmn", "Nrxn1", "Nid1", "Sfrp1")

diff_V59_astro <- list()
for (i in 1:length(Cells)) {
  cells <- subset(astro.data,
                  cell_type1 == Cells[i])
  ...
  diff_V59_astro [[i]] <- cells.diff
}
names(diff_V59_astro ) <- Cells
save(diff_V59_astro , file = "diff_V59_astro_alldiff.Rda")
...
length(diff_V59_astro)
rownames(diff_V59_astro[[1]])
ENTREZID <- bitr(rownames(diff_V59_astro[[1]][diff_V59_astro[[1]]$p_val_adj < 0.05,]), 
                 fromType = "SYMBOL", 
                 toType = "ENTREZID", 
                 OrgDb = "org.Mm.eg.db")
...
# 批量转换
scRNA.diff_V59_astro <- list()
for (i in 1:length(diff_V59_astro)) {
  ENTREZID <- bitr(rownames(diff_V59_astro[[i]][diff_V59_astro[[i]]$p_val_adj < 0.05,]), 
                   fromType = "SYMBOL", 
                   toType = "ENTREZID", 
                   OrgDb = "org.Mm.eg.db")
  ENTREZID <- ENTREZID$ENTREZID
  scRNA.diff_V59_astro[[i]] <- ENTREZID
}
names(scRNA.diff_V59_astro) <- names(diff_V59_astro)

############## 多个cluster比较 ###################
# KEGG分析
xx <- compareCluster(scRNA.diff_V59_astro, 
                     fun="enrichKEGG",
                     organism="mmu",
                     pvalueCutoff= 0.05,
                     qvalueCutoff= 0.05)
...
xx_BP <- compareCluster(scRNA.diff_V59_astro, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "BP",
                        pvalueCutoff = 0.001,
...
xx_MF <- compareCluster(scRNA.diff_V59_astro, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "MF",
                        pvalueCutoff = 0.01,
...
xx_CC <- compareCluster(scRNA.diff_V59_astro, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "CC",
                        pvalueCutoff = 0.01,
                        qvalueCutoff= 0.01) #astroglia_compare_BP
#save(xx, file = "compareCluster.GO.Rda")
...
diffgene_flna <- scRNA.diff_V59_astro[[2]]
...
diffgene_Tcf4 <- scRNA.diff_V59_astro[[3]]
...
diffgene_unknow <- scRNA.diff_V59_astro[[1]]
...
#获得基因列表
gene <- rownames(diff_V59_astro[[5]])
## 转换
gene = bitr(gene, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Mm.eg.db")
## 去重
gene <- dplyr::distinct(gene,SYMBOL,.keep_all=TRUE)
gene_df <- data.frame(logFC=diff_V59_astro[[5]]$avg_log2FC,
                      SYMBOL = rownames(diff_V59_astro[[5]]))
...
load('astro.data_8cluster_ann.Rda')
...
for (i in 1:length(Cells)) {
  ...
  diff_VFS_astro [[i]] <- cells.diff
}
names(diff_VFS_astro ) <- Cells
save(diff_VFS_astro , file = "diff_VFS_astro_alldiff.Rda")
...
length(diff_VFS_astro)
rownames(diff_VFS_astro[[1]])
ENTREZID <- bitr(rownames(diff_VFS_astro[[1]][diff_VFS_astro[[1]]$p_val_adj < 0.05,]), 
                 fromType = "SYMBOL", 
                 toType = "ENTREZID", 
                 OrgDb = "org.Mm.eg.db")
Irak3_NK.diff <- list(ENTREZID)
...
# 批量转换
scRNA.diff_VFS_astro <- list()
for (i in 1:length(diff_VFS_astro)) {
  ENTREZID <- bitr(rownames(diff_VFS_astro[[i]][diff_VFS_astro[[i]]$p_val_adj < 0.05,]), 
                   fromType = "SYMBOL", 
                   toType = "ENTREZID", 
                   OrgDb = "org.Mm.eg.db")
  ENTREZID <- ENTREZID$ENTREZID
  scRNA.diff_VFS_astro[[i]] <- ENTREZID
}
names(scRNA.diff_VFS_astro) <- names(diff_VFS_astro)

############## 多个cluster比较 ###################
# KEGG分析
xx <- compareCluster(scRNA.diff_VFS_astro, 
                     fun="enrichKEGG",
                     organism="mmu",
                     pvalueCutoff= 0.05,
                     qvalueCutoff= 0.05)
...
# GO分析
# all 整体分析会有NA出现，单独分析看看
xx_BP <- compareCluster(scRNA.diff_VFS_astro, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "BP",
                        pvalueCutoff = 0.001,
...
xx_MF <- compareCluster(scRNA.diff_VFS_astro, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "MF",
                        pvalueCutoff = 0.01,
...
xx_CC <- compareCluster(scRNA.diff_VFS_astro, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "CC",
                        pvalueCutoff = 0.005,
...
################细胞亚型数量及比例##############
plot.data <- astro.data@meta.data[c("Group", "cell_type1")]
head(plot.data)
