...
# 单核细胞亚分群
# 挑选T细胞进⾏进⼀步分群
micro1 <- row.names(zika_all_mthb_filter_SCT@meta.data)[which(zika_all_mthb_filter_SCT@meta.data$cell_type == "Microglia_Csf1r")]
micro2 <- row.names(zika_all_mthb_filter_SCT@meta.data)[which(zika_all_mthb_filter_SCT@meta.data$cell_type == "Microglia_Ctss high")]
micro3 <- row.names(zika_all_mthb_filter_SCT@meta.data)[which(zika_all_mthb_filter_SCT@meta.data$cell_type == "Microglia_Siglech")]
target.cell <- c(micro1,micro2,micro3)
length(target.cell)
micro.data <- subset(zika_all_mthb_filter_SCT, cells = target.cell)
micro.data
DefaultAssay(micro.data) <- "RNA"
micro.data <- SplitObject(micro.data,
                         split.by = "Group")
micro.data
...
# 速度较慢，样本越多越慢
# 循环运⾏
for(i in 1:length(micro.data)){
  micro.data[[i]] <- SCTransform(
    micro.data[[i]],
...
# 数据整合
# 选择⽤于整合的基因
features <- SelectIntegrationFeatures(object.list = micro.data,
                                      nfeatures = 3000)
# 准备整合
micro.data <- PrepSCTIntegration(object.list = micro.data,
                                anchor.features = features)
names(micro.data)
# 寻找锚定基因(速度很慢)
AnchorSet <- FindIntegrationAnchors(object.list = micro.data,
                                    reference = 3,
...
# 整合数据
# load("AnchorSet.Rda")
micro.data <- IntegrateData(anchorset = AnchorSet,
                           normalization.method = "SCT")
DefaultAssay(micro.data) <- "integrated"
save(micro.data, file = "micro.data.final.Rda")
############降维分析##########
rm(list = ls())
library(Seurat)
library(tidyverse)
library(stringr)
library(patchwork)
load("micro.data.final.Rda")
dim(micro.data)
# PCA分析
micro.data <- RunPCA(object = micro.data,
                    npcs = 50,
                    rev.pca = FALSE,
                    weight.by.var = TRUE,
...
ElbowPlot(micro.data,
          ndims = 50) # 寻找对细胞差异贡献度较⼤的主成分
# 选择30个主成分进⾏后续分析
# t-SNE分析，基于PCA分析结果进⾏⾮线性降维分析
micro.data <- RunTSNE(micro.data,
                     dims = 1:30)
# UMAP分析
micro.data <- RunUMAP(micro.data,
                     dims = 1:30)
# 展示PCA降维特征及其特征基因的热图
DimHeatmap(object = micro.data,
           reduction = "pca",
           dims = c(1:5),
           nfeatures = 30,
           disp.min = -2.5,
...
FeaturePlot(micro.data,
            reduction = "umap",
            features = "Cxcl2")
save(micro.data, file = "micro.data.before_cluster.Rda")
##########聚类分析##########
# KNN聚类法
...
# 把marker文件
保存下来
save(micro.data.markers, file = "micro.data.markers_cluster8.Rda")
...
write.table (sig.markers,file="PC30_micro.markers.xls",sep="\t",row.names=F,quote=F)
...
write.table (top20 ,file="PC30_top20_micro.markers.xls",sep="\t",row.names=F,quote=F)
...
DimPlot(micro.data,
  ...
  NoLegend()
#保存一下注释好的数据
save(micro.data, file = "micro.data_8cluster_ann.Rda")
load('/home/zhangc/analysis/zika/data/step6_7_micro/micro.data_8cluster_ann.Rda')

#### 保存细胞亚型 ----
saveRDS(micro.data, file = "micro_8cluster_ann.rds")

########## 单核细胞亚群的差异基因分析#############
# 批量找各个细胞在VFS中的差异基因
dput(unique(micro.data@meta.data$cell_type1))
table(micro.data@meta.data$cell_type1)
Cells <- c("Syngr1", "Cd44", "Ier2", "Ptn", "Ccr5", "Hist1h2ai", "Mrc1", 
           "Ccr2")
diff_VFS_micro <- list()
for (i in 1:length(Cells)) {
  ...
  diff_VFS_micro [[i]] <- cells.diff
}
names(diff_VFS_micro ) <- Cells
save(diff_V59_micro , file = "diff_V59_micro_alldiff.Rda")
...
c #,label_format = 60 可以修改通路一行还是两行
...
load('micro.data_8cluster_ann.Rda')
# 批量找各个细胞在VFS中的差异基因
dput(unique(micro.data@meta.data$cell_type1))
table(micro.data@meta.data$cell_type1)
Cells <- c("Syngr1", "Cd44", "Ier2", "Ptn", "Ccr5", "Hist1h2ai", "Mrc1", 
...
names(diff_VFS_micro ) <- Cells
save(diff_VFS_micro , file = "diff_VFS_micro_alldiff.Rda")
...
length(diff_VFS_micro)
rownames(diff_VFS_micro[[1]])
ENTREZID <- bitr(rownames(diff_VFS_micro[[1]][diff_VFS_micro[[1]]$p_val_adj < 0.05,]), 
                 fromType = "SYMBOL", 
                 toType = "ENTREZID", 
                 OrgDb = "org.Mm.eg.db")
Irak3_NK.diff <- list(ENTREZID)
...
# 批量转换
scRNA.diff_VFS_micro <- list()
for (i in 1:length(diff_VFS_micro)) {
  ENTREZID <- bitr(rownames(diff_VFS_micro[[i]][diff_VFS_micro[[i]]$p_val_adj < 0.05,]), 
                   fromType = "SYMBOL", 
                   toType = "ENTREZID", 
                   OrgDb = "org.Mm.eg.db")
  ENTREZID <- ENTREZID$ENTREZID
  scRNA.diff_VFS_micro[[i]] <- ENTREZID
}
names(scRNA.diff_VFS_micro) <- names(diff_VFS_micro)

############## 多个cluster比较 ###################
# KEGG分析
xx <- compareCluster(scRNA.diff_VFS_micro, 
                     fun="enrichKEGG",
                     organism="mmu",
                     pvalueCutoff= 0.05,
                     qvalueCutoff= 0.05)
...
# GO分析
# all 整体分析会有NA出现，单独分析看看
xx_BP <- compareCluster(scRNA.diff_VFS_micro, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "BP",
...
xx_MF <- compareCluster(scRNA.diff_VFS_micro, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "MF",
                        pvalueCutoff = 0.01,
...
xx_CC <- compareCluster(scRNA.diff_VFS_micro, 
                        fun = "enrichGO",
                        OrgDb = "org.Mm.eg.db",
                        ont = "CC",
                        pvalueCutoff = 0.0001,
...
################细胞亚型数量及比例##############
plot.data <- micro.data@meta.data[c("Group", "cell_type1")]
head(plot.data)
